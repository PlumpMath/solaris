...
class Planet():
    ...
    def __init__(self, app, name, model, texture=None, scale=1, distance_from_sun=0):
        self.app = app
        self.name = name
        self.model = model
    ...
    def rotateTask(self, task):
        if self.distance_from_sun != 0:
            angleDegrees = task.time * self.app.get_speed()*8
        ...
        self.planet.setHpr(task.time*self.app.get_speed()*16, 0, 0)
        return Task.cont
...
class Moon():
    ...
    def rotateTask(self, task):
        ...
        self.moon.setHpr(task.time*self.app.get_speed()*16, 0, 0)
        return Task.cont
